<template>
  ...
  >
    ...
    <div class="data" v-if="show" :style="styleObject">
      <table>
...
export default {
      ...
      if (this.currentName != this.keysearch) {
        ...
        this.dataOptions.DataRows.forEach((element) => {
          if (
            ...
            this.options.DataRows.push(element);
          }
        });
        this.show = true;
        if (Common.isNullOrUndefined(this.keysearch)) {
          this.currentId = null;
          this.currentName = null;
          this.keysearch = null;
  ...
  methods: {
    ...
    showAll(event) {
      ...
      let h = this.$refs[`${this.nameControl}_box`].clientHeight;
      let bw = this.isGroupControl
        ? this.$refs[`${this.nameControl}_button`].clientWidth + 21
        : 21;
      this.mousePointer.x = x - w + bw;
      this.mousePointer.y = y + h;
      console.log(this.mousePointer.x);

      this.styleObject = {
        width: `${w}px`,
        left: `${this.mousePointer.x}px`,
