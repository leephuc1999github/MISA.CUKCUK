...
export default {
  ...
  methods: {
     ...
     */
    handleSave(typeSave) {
      if (Common.isNullOrUndefined(this.model.NameInventoryItem)) {
        this.flagName = true;
      }
      if (Common.isNullOrUndefined(this.model.CodeInventoryItem)) {
        this.flagCode = true;
      }
      if (Common.isNullOrUndefined(this.model.UnitId)) {
        this.flagUnit = true;
      }
      if (!this.flagName && !this.flagCode && !this.flagUnit) {
        try {
          ...
          axios
            .get(
              `${urlApi}inventoryitems/duplicate?columnName=CodeInventoryItem&value=${this.model.CodeInventoryItem}&id=${this.inventoryItemId}`
            )
            .then(function (response) {
              let isDuplicate = response.data.Data;
              if (isDuplicate) {
                self.isOpenDialog = true;
                self.contentDialog = `Mã <${self.model.CodeInventoryItem}> đã tồn tại trên một trong các danh sách sau: Món ăn, đồ uống, combo, món khác, dịch vụ tính tiền theo thời gian.`;
              } else {
                if (
                  self.type == Resource.TYPE_FORM.ADD ||
                  self.type == Resource.TYPE_FORM.CLONE
                ) {
                ...
                if (self.type == Resource.TYPE_FORM.EDIT) {
                  self.editInventoryItem(
                    `${urlApi}inventoryitems/edit/${self.inventoryItemId}`,
                    formData,
                    typeSave
                  );
                }
            ...
            .catch(function (error) {
              console.log("duplicate".concat(error));
            });
        } catch (error) {
        ...
        axios
          ...
          .then(function (response) {
            ...
            if (typeSave == self.typeSave.SAVE) {
              ...
              self.$emit("typeForm", Resource.TYPE_FORM.ADD);
            }
